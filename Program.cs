...
class BloodRequest : Request
{
  public BloodRequest(int bloodRequestId, string consumerName, string consumerPhone, string division, string district, string thana, string hospital, string bloodGroup, int quantity, string date, string time)
  {
    ...
    // formating date & time to store in DateTime object
    // date formate MM/DD/YYYY
    string[] splitedDate = date.Split("-");
    int year = Convert.ToInt32(splitedDate[2]);
    int month = Convert.ToInt32(splitedDate[0]);
    int day = Convert.ToInt32(splitedDate[1]);

    // time formate HH:MM in the form of 24 hours
    string[] splitedtime = time.Split(":");
    int hour = Convert.ToInt32(splitedtime[0]);
    int minute = Convert.ToInt32(splitedtime[1]);

    this.dateTime = new DateTime(year, month, day, hour, minute, 0);
...
class Program
{
  ...
  public static void DonerBloodRequest()
  {
    ...
    Console.WriteLine("1. Accept one");
